...
try {
	...
	ON c.empleado = e.clave WHERE c.baja = 0 ORDER BY e.nombre";
    ...
    $total_cedulas_bd = $borraCed->rowCount();
...
<body>
	<div class="container my-5">
            ...
            if($_GET['pagina']>$paginas || $_GET['pagina']<=0) {
                echo "<script>location.href = 'lista_del_cedulas.php?pagina=1';</script>";
            }
            ...
            if(isset($_POST['buscar'])) {
                try {
                    $sql_ced = "SELECT c.empleado, e.nombre FROM cedulas c JOIN empleados e ON c.empleado = e.clave WHERE c.baja = 0 AND e.nombre like '%".$_POST['palabra']."%' ORDER BY e.nombre LIMIT :iniciar,:ndictamenes";
                ...
                try {
                    $sql_ced = "SELECT c.empleado, e.nombre FROM cedulas c JOIN empleados e ON c.empleado = e.clave WHERE c.baja = 0 ORDER BY e.nombre LIMIT :iniciar,:ndictamenes";
                    $sentencia_ced = $conexion->prepare($sql_ced);
                    $sentencia_ced->bindParam(':iniciar', $iniciar, PDO::PARAM_INT);
                    $sentencia_ced->bindParam(':ndictamenes', $cedulas_x_pagina, PDO::PARAM_INT);
                    $sentencia_ced->execute();
            ...
            <table class="table table-striped table-bordered">
                ...
                ?>     
                    <tr>
                        <td style="text-align: center;  width: 33.3%;"><a href="bajCedulas.php?recordID=<?php echo $cedula['empleado']; ?>"><?php echo $cedula['empleado']; ?></a></td>
            ...
            <nav aria-label="Page navigatíon example">
                <ul class="pagination">
                    <li class="page-item <?php echo $_GET['pagina']<=1 ? 'disabled' : '' ?>">
                        ...
                        <a class="page-link" href="lista_del_cedulas.php?pagina=<?php echo (int)$_GET['pagina']-1; ?>">  Anterior</a>

                    </li>

                    <?php for($i=0;$i<$paginas;$i++): ?>
                        <li class="page-item <?php echo $_GET['pagina']==$i+1 ? 'active' : '' ?>">
                            <a class="page-link" href="lista_del_cedulas.php?pagina=<?php echo $i+1; ?>">
                                <?php echo $i+1; ?>
                            </a>
                        </li>
                    <?php endfor ?>
                    ...
                    <li class="page-item <?php echo $_GET['pagina']>=$paginas ? 'disabled' : '' ?>">
                        <a class="page-link" href="lista_del_cedulas.php?pagina=<?php echo (int)$_GET['pagina']+1; ?>">Siguiente</a>
                    </li>
                </ul>
            </nav>    
        </div>
